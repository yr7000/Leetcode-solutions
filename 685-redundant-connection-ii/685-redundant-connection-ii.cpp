...
public:
    ...
    vector<int> findRedundantDirectedConnection(vector<vector<int>>& edges) {
        ...
        int nodeHasIndegreeTwo = -1;
        for(auto edge:edges){
            indegree[edge[1]-1]++;
            if(indegree[edge[1]-1] == 2){
                nodeHasIndegreeTwo = edge[1];
                break;
            }
        }
        if(nodeHasIndegreeTwo == -1) return detectCycle(edges,{});
        for(int i = n-1;i>=0;i--){
            if(edges[i][1] == nodeHasIndegreeTwo){
                if(detectCycle(edges,edges[i]).empty()) return edges[i];
            }
        }
        return {};
    }
